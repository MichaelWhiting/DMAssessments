...
function getNames(people) {
  ...
  people.forEach((person) => {
    attachedNames.push(`${person.firstName} ${person.lastName}`);
  });
...
function translateToPirateTalk(phrase) {
  ...
  words.forEach((word, i) => {
    // checks if the word is one of the key/value pairs in the object
    if (EN_PIRATE_LOOKUP[word] !== undefined) {
      // if so, replace it with the value of that key.
      words[i] = EN_PIRATE_LOOKUP[word];
    }
  });

  // return the array joined together by spaces.
  return words.join(" ")
}
...
//
// Ex.:
//   wordCount('hello world hello world hello world ')
//   => { hello: 1, world: 1 }
